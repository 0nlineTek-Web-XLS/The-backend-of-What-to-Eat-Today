...
def add_canteen(data: CanteenBase, db = Depends(get_db)):
    return canteen.add(db, data)

@router.delete("/{canteen_id}", response_model=dict[str, str])
def delete_canteen(canteen_id: int, db = Depends(get_db)):
    return canteen.delete(db, canteen_id)

@router.put("", response_model=dict[str, str])
def update_canteen(data: CanteenItem, db = Depends(get_db)):
    try:
