...
def update(db: Session, data: CanteenItem) -> dict[str, str]:
    ...
    Update a canteen.
    """
    db_canteen: Canteen | None = (
        db.query(Canteen).filter(Canteen.id == data.id).first() if data.id else None
    )
...
def update_floor_info(db: Session, floor: FloorData) -> dict[str, str]:
    ...
    db_floor: Floor | None = (
        db.query(Floor)
        .filter(
            and_(Floor.canteen == floor.canteen, Floor.floor_in_canteen == floor.floor)
        )
        .first()
    )
