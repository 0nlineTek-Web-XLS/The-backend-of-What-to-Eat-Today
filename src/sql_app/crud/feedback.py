...
def remove_reply(db: Session, fid: int):
    db_feedback = db.query(Feedback).filter(Feedback.id == fid).first()
    assert db_feedback, "No such feedback"
    db_feedback.reply = None
    db_feedback.reply_time = None
    db_feedback.reply_uid = None
    db.refresh(db_feedback)
    db.commit()
    return db_feedback
...
def modify_content(db: Session, content: FeedbackModify, uid: int):
    db_feedback: Feedback | None = (
        db.query(Feedback).filter(Feedback.id == content.id).first()
    )
    assert db_feedback, "No such feedback"
    if db_feedback.user_id != uid:
        raise ValueError("Unauthorized")
    # check if has been replied
    if db_feedback.reply:
...
def get_by_target(db: Session, tid: int, filter_replied: bool | None = None, skip: int = 0, limit: int = 100):
    r = db.query(Feedback).filter(Feedback.towards == tid)
    # no filter, return all
    if filter_replied is None:
        return r.offset(skip).limit(limit).all()

    if filter_replied:
        return r.filter(not_(Feedback.content == None)).offset(skip).limit(limit).all()
    # filter non-replied ones
    return r.filter(Feedback.content == None).offset(skip).limit(limit).all()
